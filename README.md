...
loss_persentropy0(point_cloud, dgm, true_dgm, delta)
loss_persentropy1 (point_cloud, dgm, true_dgm, delta)
loss_dsigma0(point_cloud, point_cloud2, dgm, dgm2, sigma=0.05),
loss_dsigma1(point_cloud, point_cloud2, dgm, dgm2, sigma=0.05)
loss_density(point_cloud, point_cloud2, dgm, dgm2, sigma=0.2, scale=0.002, maxrange=35., npoints=30)
loss_push0(point_cloud, dgm)
```
The diagram can be generated by directly calling
```
get_dgm(point_cloud, deg=1)
```
Where the shape of the point cloud is expected to be (N, D) (N: number of points, D: dimension of each point). Additionally, deg is the homology degree (0 or 1) and not specifying it sets it as 1 (the more general option). 
...
```
dgm_true can be set as None, and it will be directly generated by the function. Additionally, topo_weights is the list [w_bottleneck0,w_bottleneck1,w_entropy0,w_entropy1,w_ksigma0,w_ksigma1,w_density]; each element is the weight asociated to a topological loss. In order to not use a function, set its weight to 0.
