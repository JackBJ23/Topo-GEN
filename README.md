...
In particular, we introduce a new family of topological regularizers that can be implemented into the training process of any generative model. More in general, they can be implemented as loss functions in any machine learning problem that involves learning to generate a point cloud from another input point cloud, regardless of their dimensions or number of points. In fact, a key aspect of persistence diagrams is that they translate the topological and geometrical features of different point clouds into a single object that can be efficiently compared through the topological regularizers, even if the two point clouds lie in different spaces and have different number of points.
